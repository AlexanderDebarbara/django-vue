...
workon filmes  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development filmes npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop filmes
    docker rm filmes
    docker run --name filmes -d --env-file /home/ubuntu/filmes.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/filmes:/dkdata \
        filmes start_web.sh
}
...
echo_green "Welcome to filmes's dev env"
